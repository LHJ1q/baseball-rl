...
| **Repertoire mask timing + default** | Canonical Q-Transformer doesn't have the concept | Inference-only AND **disabled by default** (hyperparameter `repertoire_mask_min_count`, default 0). FQE / behavioral / `policy()` all skip masking unless explicitly enabled via CLI. IQL training is unaffected regardless. | Arsenal data is noisy for low-sample pitchers; the pitcher embedding subsumes the same info; IQL's expectile-V is the soft conservatism mechanism. Hyperparameter knob lets you experiment with-vs-without on a trained checkpoint. Set `--repertoire-mask-min-count N` (e.g., 5) on `scripts/09_behavioral.py` or `scripts/10_fqe.py` to enable. |
